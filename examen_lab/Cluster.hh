...
class Cluster {
    ...
    */
    ...
    void podar_cluster(int k, int& error);

    private:

    static void aux_podar_cluster(BinTree<map<string, Procesador>::iterator>& clust, map<string, Procesador>& procesador, int k, int& num);

    static int count_delete(const BinTree<map<string, Procesador>::iterator>& clust, map<string, Procesador>& procesador);
