...
void Cluster::podar_cluster(int k, int& error) {
    if (cluster.value()->second.consultar_num_procesos() < k) error = 9;
    else {
        error = -1;
        int num = 0;
        aux_podar_cluster(cluster, Procesadores, k, num);
        cout << "Poda " << num << endl;
    }
}

void Cluster::aux_podar_cluster(BinTree<map<string, Procesador>::iterator>& clust, map<string, Procesador>& procesador, int k, int& num) {
    if (not clust.empty()) {
        if (clust.value()->second.consultar_num_procesos() < k) {
            num += (count_delete(clust.left(), procesador) + count_delete(clust.right(), procesador) + 1);
            procesador.erase(clust.value());
            clust = BinTree<map<string, Procesador>::iterator>();
        }
        else {
            BinTree<map<string, Procesador>::iterator> left = clust.left();
            BinTree<map<string, Procesador>::iterator> right = clust.right();
            aux_podar_cluster(left, procesador, k, num);
            aux_podar_cluster(right, procesador, k, num);
            clust = BinTree<map<string, Procesador>::iterator>(clust.value(), left, right);
        }
    }
}

int Cluster::count_delete(const BinTree<map<string, Procesador>::iterator>& clust, map<string, Procesador>& procesador) {
    if (not clust.empty()) {
        procesador.erase(clust.value());
        return 1 + count_delete(clust.left(), procesador) + count_delete(clust.right(), procesador);
    }
    return 0;
}
...
void Cluster::procesador_apto(const BinTree<map<string, Procesador>::iterator>& clust, map<string, Procesador>::iterator& procesador, int& hueco_min, int prof_actual, int& prof_procesador, Proceso& proc) {
    if (not clust.empty()) {
        ...
        int hueco_procesador = clust.value()->second.hueco_max(proc.consultar_memoria());
    ...
    if (not clust.empty()) {
        if ((*clust.value()).first == ident) {
            if (clust.right().empty() and clust.left().empty()) {
                ...
                BinTree<map<string, Procesador>::iterator> clust_aux; // Cigarros
                leer_cluster(clust_aux); // Clust_aux = Clust
                clust = BinTree<map<string, Procesador>::iterator>(clust_aux.value(), clust_aux.left(), clust_aux.right()); // Cigarros
            }
        } // Poner con else. Si se ha encontrado el procesador, por mucho que busque, no lo voy a volver a encontrar.
        BinTree<map<string, Procesador>::iterator> left = clust.left();
        BinTree<map<string, Procesador>::iterator> right = clust.right();
